...
- [x] **M6 quality pass** - **stage: polish** - Add linting, API/UI regression coverage, and a concise demo seed reset flow. Acceptance: `python -m pytest`
...
- 2026-09-24 - M6 completed: added ESLint (`npm run lint`), a `python -m backend.reset_demo` seed reset flow with regression tests, and a README demo section.
...
- 2026-09-24 - Reset deletes every row from the five workflow tables and re-seeds, rather than deleting the database file, so it works while the API holds the file open and never removes anything outside the local demo tables. Lint covers `src` only to keep the check fast.
